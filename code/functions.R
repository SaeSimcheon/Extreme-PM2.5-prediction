...
validation_tool<-function(true_y, pred_y, threshold=76,beta = 2){
  
  result<-list()
  tb1=table( ifelse(pred_y <threshold, 0, 1)  , ifelse(true_y<threshold, 0, 1)  )
  amount = beta**2
  result$F2score = ((1+amount)*(tb1[2,2]))/((1+amount)*(tb1[2,2])+amount*tb1[1,2]+tb1[2,1])
  result$senc=tb1[2,2]/ sum(tb1[,2])
  result$spec=tb1[1,1]/ sum(tb1[,1])
  result$PPV=tb1[2,2]/ sum(tb1[2,])
  result$NPV=tb1[1,1]/ sum(tb1[1,])
...
voting_validation_tool<-function(true_y, pred_y, threshold=76,beta = 2){
  result<-list()
  outmat = matrix(NA,dim(pred_y)[1],dim(pred_y)[2])
  
  for (i in 1:dim(pred_y)[2]){
    outmat[,i]=ifelse(pred_y[,i] <threshold, 0, 1)
  }
  
  pred= apply(outmat,MARGIN = 1,function (x) ifelse(sum(x) <(dim(pred_y)[2] %/% 2)+1,0,1))
  tb1=table( pred , ifelse(true_y<threshold, 0, 1)  )
  amount = beta**2
  result$F2score = ((1+amount)*(tb1[2,2]))/((1+amount)*(tb1[2,2])+amount*tb1[1,2]+tb1[2,1])
  result$senc=tb1[2,2]/ sum(tb1[,2])
  result$spec=tb1[1,1]/ sum(tb1[,1])
...
ThreeStage_lasso_cv<-function (y, x, xstar, tau.e, grid.lam =seq(-0.5, 1.5, 0.1), grid.k, 
                               tau.lam, a = 0, tol = 1e-02,cv_type="rmse") 
{
  x = as.matrix(x)
  n = length(y)
  p = ncol(x)
